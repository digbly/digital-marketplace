...
const VendorSettingsForm: React.FC<VendorSettingsFormProps> = ({ vendor }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification(
        'error',
        error?.data?.message || 'Failed to update store settings. Please try again.'
      );
    }
  };
  ...
  return (
    <div className="max-w-3xl space-y-8">
      ...
      {isFetching ? (
        ...
        <VendorSettingsForm key={vendor.id || 'default'} vendor={vendor} />
      ) : (
        <div className="p-8 rounded-3xl bg-slate-900 border border-slate-800 text-slate-400 text-xs text-center">
