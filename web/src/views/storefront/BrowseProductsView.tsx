  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">{t('browse.title')}</h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            {t('browse.subtitle', { count: meta?.total ?? filteredProducts.length })}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-300">
            ...
            <span className="text-slate-400">{t('storefront.sortBy')}:</span>
            <select
              value={sortBy}
              onChange={(e) => {
            ...
            >
              <option value="newest" className="bg-slate-900 text-white">{t('storefront.sortNewest')}</option>
              <option value="popular" className="bg-slate-900 text-white">{t('storefront.sortPopular')}</option>
              <option value="rating" className="bg-slate-900 text-white">{t('storefront.sortRating')}</option>
              <option value="price_asc" className="bg-slate-900 text-white">{t('storefront.sortPriceLow')}</option>
              <option value="price_desc" className="bg-slate-900 text-white">{t('storefront.sortPriceHigh')}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="space-y-6">
          ...
          <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-3">
            <h3 className="text-xs font-bold uppercase text-slate-300 tracking-wider">{t('browse.searchAssets')}</h3>
          ...
          {/* Product Type Filter */}
          <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-3">
            <h3 className="text-xs font-bold uppercase text-slate-300 tracking-wider">{t('browse.assetFormat')}</h3>
            <div className="space-y-1">
              {[
                { id: 'all', label: t('browse.allFormats'), icon: Layers },
                { id: 'downloadable_file', label: t('browse.downloadableFiles'), icon: DownloadCloud },
                { id: 'license_key', label: t('browse.softwareLicenses'), icon: Key },
                { id: 'bundle', label: t('browse.bundles'), icon: ShoppingBag },
              ].map((item) => (
                <button
                  key={item.id}
                  onClick={() => {
          ...
          <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase text-slate-300 tracking-wider">{t('browse.categories')}</h3>
              {selectedCategory && (
                <button
                  onClick={() => {
                    setSelectedCategory(null);
          ...
          {/* Rating Filter */}
          <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-3">
            <h3 className="text-xs font-bold uppercase text-slate-300 tracking-wider">{t('browse.minRating')}</h3>
            <div className="space-y-1">
              {[
                { rating: 0, label: t('browse.allRatings') },
                { rating: 4.5, label: t('browse.ratingAndUp', { rating: '4.5' }) },
                { rating: 4.0, label: t('browse.ratingAndUp', { rating: '4.0' }) },
              ].map((r) => (
                <button
                  key={r.rating}
        ...
        <div className="lg:col-span-3 space-y-6">
          {isProductsLoading ? (
            ...
            <div className="py-20 text-center rounded-3xl bg-slate-900/50 border border-slate-800 space-y-3">
              ...
              <h3 className="text-base font-bold text-slate-200">{t('storefront.noProductsFound')}</h3>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 pt-6 border-t border-slate-800">
                  ...
                  <span className="text-xs text-slate-400 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800">
                    {t('common.page')} <strong className="text-white">{page}</strong> {t('common.of')} <strong className="text-white">{totalPages}</strong>
                  </span>
                  <button
